...
##################################################
# Descripción del script: Ejecutar un análisis de vulnerabilidades de CPU. También descarga y ejecuta un comprobador de vulnerabilidades de CSME para equipos con chipset Intel.
...
cd /tmp
rm -r /tmp/cpuvuln 1>/dev/null 2>/dev/null
...
# Descarga de los comprobadores de vulnerabilidades de CPU, y vulnerabilidades de CSME.
...
then
	wget https://downloadmirror.intel.com/28632/CSME_Version_Detection_Tool_Linux_15_0_4_22.tar.gz 1>/dev/null 2>/dev/null
fi
clear
...
if [ $procesador = GenuineIntel ];
then
	tar -xf CSME_Version_Detection_Tool_Linux_15_0_4_22.tar.gz
